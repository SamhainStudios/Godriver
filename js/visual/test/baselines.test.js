...
import { BaselineStore, assertMatchesBaseline, BaselineMissingError, BaselineMismatchError, solidPng } from "../src/index.js";

async function tempStore() {
	const root = await mkdtemp(join(tmpdir(), "godriver-visual-"));
	return {
		store: new BaselineStore({ dir: join(root, "baselines"), artifactsDir: join(root, "artifacts", "visual") }),
		root,
	};
}
...
test("first run without baseline and without UPDATE_BASELINE throws BaselineMissingError with guidance", async () => {
	...
	try {
		...
		await assert.rejects(
			...
			/** @param {Error} err */
			(err) => err instanceof BaselineMissingError && /UPDATE_BASELINE=true/.test(err.message),
		);
	} finally {
		await rm(root, { recursive: true, force: true });
	}
...
test("first run with UPDATE_BASELINE=true writes baseline + sidecar and passes", withEnv({ UPDATE_BASELINE: "true" }, async () => {
	const { store, root } = await tempStore();
	try {
		const png = await solidPng(32, 32, "#336699");
		const result = await assertMatchesBaseline(store, "created", { screenshot: captureFn(png), driverLabel: "godot-4.7.2" });
...
test("UPDATE_BASELINE=true regenerates an existing baseline", withEnv({ UPDATE_BASELINE: "true" }, async () => {
	const { store, root } = await tempStore();
	try {
		await store.write("regen", await solidPng(32, 32, "#336699"), { driver: "old" });
		const png = await solidPng(32, 32, "#ff0000");
		const result = await assertMatchesBaseline(store, "regen", { screenshot: captureFn(png) });
...
test("identical capture passes and writes nothing to artifacts", async () => {
	...
	try {
		...
		await assert.rejects(() => readFile(join(store.artifactDir("same"), "actual.png")), /ENOENT/);
	} finally {
		await rm(root, { recursive: true, force: true });
	}
});
...
test("injected pixel difference fails with actual/diff/report artifacts", async () => {
	...
	try {
		...
		const patch = await sharp({ create: { width: 1, height: 1, channels: 3, background: "#ff0000" } }).png().toBuffer();
		...
		assert.ok((report.match(/data:image\/png;base64,/g) ?? []).length >= 3, "report embeds all three images");
...
test("GODRIVER_DIFF_OUTPUT=1 writes diff artifacts even on success", withEnv({ GODRIVER_DIFF_OUTPUT: "1" }, async () => {
	const { store, root } = await tempStore();
	try {
		const png = await solidPng(32, 32, "#336699");
		await store.write("debug", png, {});
		const result = await assertMatchesBaseline(store, "debug", { screenshot: captureFn(png) });
		assert.equal(result.match, true);
