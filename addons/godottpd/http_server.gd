...
# GTD-003 deviation patch: per-client Mutex guarding all StreamPeerTCP access.
# godottpd upstream writes responses from worker threads while the main thread
# polls/reads the same peers — unsynchronized concurrent access. Keys are the
# StreamPeerTCP objects (reference keys), values are Mutex.
...
func _process(_delta: float) -> void:
	...
	for client in self._clients:
		# GTD-003: guard main-thread socket reads with the per-client mutex.
		var lock: Mutex = _lock_for(client)
		lock.lock()
		client.poll()
		var status: int = client.get_status()
		var bytes: int = client.get_available_bytes() if status == StreamPeerTCP.STATUS_CONNECTED else 0
		var request_string: String = client.get_utf8_string(bytes) if bytes > 0 else ""
		lock.unlock()
		if status == StreamPeerTCP.STATUS_CONNECTED and bytes > 0:
			self._handle_request(client, request_string)
	_remove_disconnected_clients()


## GTD-003 deviation patch: return (creating if needed) the Mutex guarding a
## client's StreamPeerTCP. See _peer_locks.
func _lock_for(client: StreamPeer) -> Mutex:
	var lock: Mutex = _peer_locks.get(client)
	if lock == null:
		lock = Mutex.new()
		_peer_locks[client] = lock
	return lock


func _remove_disconnected_clients():
	var valid_statuses = [StreamPeerTCP.STATUS_CONNECTED, StreamPeerTCP.STATUS_CONNECTING]
	# GTD-003: status reads are guarded by the per-client mutex (workers may
	# still be writing to a peer being reaped).
	self._clients = self._clients.filter(
		func(c: StreamPeerTCP):
			var lock: Mutex = _lock_for(c)
			lock.lock()
			var status: int = c.get_status()
			lock.unlock()
			return valid_statuses.has(status)
	)
	# GTD-003: drop mutexes for reaped clients.
	for client in _peer_locks.keys():
		if not _clients.has(client):
			_peer_locks.erase(client)
...
func __perform_current_request(client: StreamPeer, request: HttpRequest):
	...
	# GTD-003 deviation patch: hand the worker the per-client mutex so response
	# writes are serialized against main-thread reads of the same peer.
	response.client_lock = _lock_for(client)
